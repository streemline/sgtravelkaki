...
def bus_stop(bot, update):
    ...
    if len(split_arr) == 2:
        ...
        msg += '*Example* : \n- /busstop 67329 \n- /busstop 67329 163 \n\n'
        msg += 'You can also send me a inline query by typing @oibuslehbot followed '
...
def inline_button(bot, update):
    ...
    if data_arr[0] == 'refresh':
        if len(data_arr) == 2:
            botan.track(BOTAN_TOKEN, callback.message.chat.id, callback, 
            'inline-btn-refresh-all')
            msg = bus.get_bus_arrival_msg(data_arr[1])
            reply_markup = bus_arrival_markup(data_arr[1])
        else:
            botan.track(BOTAN_TOKEN, callback.message.chat.id, callback, 
            'inline-btn-refresh-single')
        ...
        if bus_stop_detail is not None:
            ...
            botan.track(BOTAN_TOKEN, callback.message.chat.id, callback, 
            'inline-btn-streetview-not-found')
